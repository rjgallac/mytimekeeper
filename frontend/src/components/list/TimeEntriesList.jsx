...
/**
 * List component displaying time entries in a table with week navigation
 */
const TimeEntriesList = ({ entries, weekStart, setWeekStart, handleEdit, handleDelete }) => {
  const prevWeek = () => {
              ...
              <tr key={item.id} className="border-b hover:bg-accent">
                ...
                <td className="py-2 px-4">
                  <div className="flex gap-2">
                    ...
                    <Button onClick={() => handleDelete(item.id)} variant="ghost" size="sm" className="text-destructive hover:text-destructive">Delete</Button>
                  </div>
                </td>
              </tr>
            ))}
