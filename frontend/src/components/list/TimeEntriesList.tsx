  ...
  return (
    <div className="">
      ...
      <div className="flex justify-between items-center mb-4">
        <Button onClick={prevWeek} variant="green" size="sm">
          &larr; Previous week
        </Button>
        <span className="text-sm text-muted-foreground">
          Week starting {toDateLocal(weekStart).toLocaleDateString()}
        </span>
        <Button onClick={nextWeek} variant="green" size="sm">
          Next week &rarr;
        </Button>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {entries.map((item: TimeEntry, index: number) => (
              <tr
                key={item.id ?? `temp-${index}-${item.date}`}
                className="border-b hover:bg-accent"
              >
                <td className="py-2 px-4">
                  {toDateLocal(item.date).toLocaleDateString("en-GB", {
                    weekday: "short",
                  })}
                  &nbsp;
                ...
                <td className="py-2 px-4">
                  {item.morning_start?.slice(0, 5) || ""} -{" "}
                  {item.morning_end?.slice(0, 5) || ""}
                </td>
                <td className="py-2 px-4">
                  {item.afternoon_start?.slice(0, 5) || ""} -{" "}
                  {item.afternoon_end?.slice(0, 5) || ""}
                </td>
                <td className="py-2 px-4">
